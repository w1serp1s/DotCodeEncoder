...
/*-------------------------------------------------------------------------*/
void Usage(void) {
 printf("\nCommand line: \"DotCode File [/x# /u# /h# /w# /q# /d# /l /s /p]\"\n");
...
// printf("       /m# specifies symbol Mask 1-4 (default is Best Mask)\n");
 ...
 printf("Output is \"DotCode.bmp\".  [Copyright 2008 by Honeywell Imaging & Mobility]");
}
...
void NextBit (int obt, int *q, FILE *f) {
	*q += *q + (1-obt);
	if (*q > 255) { fprintf(f,"%c",*q-256); *q = 1; }
}
...
/*** ... and output this row "xdim" times ***/
		for (j=0; j<xdim; j++) {
			...
			ydis = (j<<1) - (xdim-ucut-1); if (ydis < 0) ydis = -ydis;
			for (k=0; k<NCOL; k++) {
...
/*** ... and output it "xdim" times, ***/
				for (l=0; l<xdim; l++) {
					xdis = (l<<1) - (xdim-ucut-1); if (xdis < 0) xdis = -xdis;
...
/*** & finally padding each row to a multiple of 4 bytes! ***/
			...
			k %= 4; if (k) { for (; k<4; k++) fprintf(ofile,"%c",0); }
		}
...
static void PlotSymbol (output *out) {
	...
	for (i=k=0; i<NROW; i++) {
		for (j=0; j<NCOL; j++) {
			if (!(j%8)) { c = BMAP[k++]; b = 128; }
			if (j < 80) {
				if (c&b) printf("O"); else printf(" ");
				b >>= 1;
			}
		}
		if (j < 80) printf("\n");
...
int main (int argc, char *argv[]) {
	int i, ucut, xdim, hgt, wid, dots, lit, msk, qz, show, plot, ok;
	UCHAR fname[250];

// Default all of the local and input parameters:
	ucut = show = plot = hgt = wid = lit = 0; xdim = 5; qz = 3; msk = -1; dots = ok = 1;

// Then parse the command line for all arguments:
	if (argc > 1) strcpy(fname,argv[1]); else { printf("\nFileName is Required!\n"); ok = 0; }
	for (i=2; (ok) && (i < argc) && (strchr("-/",argv[i][0]) != 0); i++) {
		switch (argv[i][1]) {
			case 'X': case 'x':	xdim = atoi(argv[i]+2); break;
			case 'U': case 'u':	ucut = atoi(argv[i]+2); break;
			case 'H': case 'h':	hgt = atoi(argv[i]+2); break;
			case 'W': case 'w':	wid = atoi(argv[i]+2); break;
			case 'M': case 'm':	msk = atoi(argv[i]+2); break;
			case 'Q': case 'q':	qz = atoi(argv[i]+2); break;
			case 'D': case 'd':	dots = atoi(argv[i]+2); break;
			case 'L': case 'l':	lit = 1; break;
			case 'S': case 's':	show = 1; break;
			case 'P': case 'p':	plot = 1; break;
...
// ...& now check that the arguments are all valid:
	if (ok) {
		if (xdim < 1) { printf("\nX-dimension too Low!\n"); ok = 0; }
		if ((ucut < 0)||(ucut >= xdim)) { printf("\nUnachieveable Undercut!\n"); ok = 0; }
		if ((!wid)&&(hgt)&&(hgt < 5)) { printf("\nSymbol Height too Low!\n"); ok = 0; }
		if ((!hgt)&&(wid)&&(wid < 7)) { printf("\nSymbol Height too Low!\n"); ok = 0; }
		if (!TWIX(-1,3,msk)) { printf("\nIllegal Mask Value!\n"); ok = 0; }
	}

	if (ok) {
// OK so far?... then accept the data message:
		inputs in; output OUT, *out = &OUT; UCHAR msg[4000];
		if ((*fname == '-')||(*fname == '/')) strcpy(msg,fname+1);
		else {
			FILE *f = fopen(fname,"rb");
			if (f) {
				i = fread(msg,sizeof(char),4000,f); fclose(f); msg[i] = 0;
			} else { printf("\nCan't Open \"%s\"\n",fname); ok = 0; }
		}

		if (ok) {
// & load up the "inputs" structure
			in.msg = msg; in.msglen = strlen(msg); in.hgt = hgt; in.wid = wid;

if (show) { printf("Input Data: "); for (i=0; i<in.msglen; i++) printf(" %d",msg[i]); printf("\n"); }

// & if so, go find out how big this symbol must be
			i = DotCodeEncode(&in,&OUT,lit,msk,0,0);

			if (i >= 0) {
				BMAP = (UCHAR*)malloc(sizeof(UCHAR) * i);
				if (BMAP) {
					DotCodeEncode(&in,&OUT,lit,msk,1,show);
	...
	if (ok) return 1; else { Usage(); return 0; }
}
